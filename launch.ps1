...
function Test-LocalApp {
  ...
  try {
    ...
    return (
      ...
      $Response.app -eq "kindle-cards" -and
...
if ($NodeVersion -lt [Version]"22.22.2") {
  throw "Kindle Cards requires Node.js 22.22.2 or newer. Current version: $NodeVersionText"
}
...
if (-not $Port) {
  throw "No available local port found in 4310-4319. Close an older Kindle Cards window and retry."
}

$Url = "http://127.0.0.1:$Port/"

if (-not (Test-LocalApp -Url $Url -ExpectedBuildId $BuildId)) {
  $LogPath = Join-Path $ProjectRoot "kindle-cards-server-$Port.log"
  $ErrorLogPath = Join-Path $ProjectRoot "kindle-cards-server-$Port-error.log"
  Start-Process `
    -FilePath $NodePath `
  ...
  if (-not $Ready) {
    throw "Kindle Cards did not start. See $LogPath and $ErrorLogPath"
  }
}
...
Write-Host "Kindle Cards ready at $Url"
